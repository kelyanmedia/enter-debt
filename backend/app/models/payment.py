...
class Payment(Base):
    ...
    quality_ok = Column(Boolean, nullable=False, default=True)
    quality_fail_reason = Column(Text, nullable=True)
    nps_score = Column(Integer, nullable=True)
    portfolio_case = Column(Boolean, nullable=True)
    deadline_shift_reason = Column(Text, nullable=True)
    pm_commission_rate = Column(Numeric(5, 2), nullable=True)
