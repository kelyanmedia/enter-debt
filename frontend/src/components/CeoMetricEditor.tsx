...
export function CeoMetricEditModal({
  ...
  const title =
    metric === 'client_history'
      ? `Активные партнёры по месяцам · ${year}`
      : metric === 'turnover'
  ...
  const sub =
    metric === 'client_history'
      ? 'Целые числа — партнёры за месяц (те же линии, что на карточках CEO: веб, SEO, PPC, приложения, техподдержка). Сохраняется как ручная правка поверх расчёта из базы.'
      : metric === 'turnover'
