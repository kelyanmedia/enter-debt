...
export default function FinancePlPage() {
  ...
  return (
    <Layout>
      <PageHeader
        title="P&L"
        subtitle="Выручка по категориям проектов (факт оплат по графику). Расходы ДДС — по категориям, в т.ч. «Агаси Д (дивиденды)» для личного вывода. Строка «Зарплатный фонд» — выплаты сотрудникам из «Команда» (USD и UZS) и зарплатные статьи ДДС; USD умножаются на курс «Курс для P&L» в ДДС для того же месяца, что колонка отчёта (если курс за месяц не 0). В итоге: операционный результат без Агаси Д; «Чистая прибыль» — только суммы Агаси Д из ДДС."
      />
      <div
        style={{
          flex: 1,
      ...
      >
        ...
        <p style={{ fontSize: 12, color: '#64748b', lineHeight: 1.55, margin: 0, maxWidth: '100%' }}>
          ...
          Строка «Процент менеджера (комиссии)» — суммы из раздела «Комиссия» по месяцу даты проекта (прибыль × % менеджера).
          Зарплатный фонд объединяет выплаты в «Команда» и расходы ДДС с категорией «Зарплата». Не дублируйте в ДДС приход то,
          что уже отражено оплатами по проектам, если не нужен отдельный кассовый учёт.
        </p>
      </div>
    </Layout>
  )
